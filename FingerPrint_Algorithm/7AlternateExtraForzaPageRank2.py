...
def calculate_topics_for_each_message():
    ...
    Updates the topicalmatrix with counts for topics that appeared before.
    """
    global dictionary_seed_conversation_and_generated_chain
    ...
    for topic_id, convo in dictionary_seed_conversation_and_generated_chain.items():
        # Combine the messages in the conversation into one large text block
        total = "\n".join(
            processed_conversation_blocks[entry["message_id"]] for entry in convo
        )
        
        # Extract the topics from the conversation
        ...
        #print(f"Processed conversation {i+1} with topic count updates.")
        i += 1
    
    # Save the topical matrix to a temporary file for analysis
    save_topical_matrix_to_temp_file()


def save_topical_matrix_to_temp_file():
    """
    Saves the topical matrix to a temporary file for analysis, with subkeys sorted by their values.
    """
    global topicalmatrix

    # Sort the subkeys of each topic by their values
    sorted_topicalmatrix = {
        key: dict(sorted(subitems.items(), key=lambda item: item[1], reverse=True))
        for key, subitems in topicalmatrix.items()
    }


    # Step 2: Sort the top-level keys of the topicalmatrix by the number of subkeys
    sorted_topicalmatrix = dict(
        sorted(sorted_topicalmatrix.items(), key=lambda item: len(item[1]), reverse=True)
    )

    # Update the global topicalmatrix with the sorted structure
    topicalmatrix = sorted_topicalmatrix

    # Use tempfile to create a temporary file
    with tempfile.NamedTemporaryFile(delete=False, mode='w', newline='', encoding='utf-8') as temp_file:
        # Save the sorted topical matrix as a JSON string
        temp_file.write(json.dumps(sorted_topicalmatrix, indent=4))
        
        # Print the file path for reference (if needed for debugging)
        print(f"Topical matrix saved to: {temp_file.name}")
